...
public class HelloSensorsAnalytics {
  ...
  public static void main(String[] args) throws Exception {
    SensorsDataAPI.sharedInstanceWithConfigure(
        "http://aws03:8006/sa",     // 日志服务器地址
        5000,                       // 日志发送时间间隔，单位毫秒
        100,                        // 最大缓存条目数，达到后立刻发送
        true                        // 是否Debug模式，该模式下会输出调试日志
    );
    ...
    */
    ...
    SensorsDataAPI.sharedInstance().track(cookieId, "ViewHomePage", properties); // 记录访问首页这个event
    ...
    SensorsDataAPI.sharedInstance().track(cookieId, "SearchProduct", properties);      // 记录搜索商品这个event
    ...
    SensorsDataAPI.sharedInstance().track(cookieId, "ViewProduct", properties);      // 记录浏览商品这个event
    ...
    SensorsDataAPI.sharedInstance().trackSignUp(registerId, cookieId);
    ...
    SensorsDataAPI.sharedInstance().profileSet(registerId, profiles);    // 此时传入的是注册ID了
    ...
    SensorsDataAPI.sharedInstance().track(registerId, "SubmitOrder", properties); // 注意，此时使用的已经是注册ID了
    // 订单中手机这个商品的信息
    properties.clear();
    properties.put("$os", "Windows");         // 通过请求中的UA，可以解析出用户使用设备的操作系统是windows的
    properties.put("$os_version", "8.1");     // 操作系统的具体版本
    ...
    SensorsDataAPI.sharedInstance().track(registerId, "SubmitOrderDetail", properties);
    // 订单中手机膜这个商品的信息
    properties.clear();
    properties.put("$os", "Windows");         // 通过请求中的UA，可以解析出用户使用设备的操作系统是windows的
    properties.put("$os_version", "8.1");     // 操作系统的具体版本
    ...
    SensorsDataAPI.sharedInstance().track(registerId, "PayOrder", properties);
    ...
    SensorsDataAPI.sharedInstance().track(registerId, "PayOrderDetail", properties);
    ...
    SensorsDataAPI.sharedInstance().track(registerId, "CancelOrder", properties);
    // 手机这个商品的取消情况
    properties.clear();
    properties.put("$os", "Windows");         // 通过请求中的UA，可以解析出用户使用设备的操作系统是windows的
    properties.put("$os_version", "8.1");     // 操作系统的具体版本
    ...
    properties.put("CancelReason", "地址填写错误"); // 取消订单的原因
    properties.put("CancelTiming", "AfterPay");   // 取消订单的时机
    SensorsDataAPI.sharedInstance().track(registerId, "CancelOrderDetail", properties);
    // 手机膜这个商品的取消情况
    properties.clear();
    properties.put("$os", "Windows");         // 通过请求中的UA，可以解析出用户使用设备的操作系统是windows的
    properties.put("$os_version", "8.1");     // 操作系统的具体版本
    ...
    SensorsDataAPI.sharedInstance().track(registerId, "ReceiveProduct", properties);
    // 手机膜这个商品的配送情况
    properties.clear();
    properties.put("$os", "Windows");         // 通过请求中的UA，可以解析出用户使用设备的操作系统是windows的
    properties.put("$os_version", "8.1");     // 操作系统的具体版本
    ...
    SensorsDataAPI.sharedInstance().track(registerId, "ServiceAfterSale", properties);
    ...
    SensorsDataAPI.sharedInstance().shutdown();   // 关闭API，关闭时会自动调用flush
  }
