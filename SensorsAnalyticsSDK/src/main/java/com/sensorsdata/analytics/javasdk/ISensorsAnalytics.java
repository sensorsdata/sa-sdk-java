...
public interface ISensorsAnalytics {
     ...
     * 设置每个事件都带有的一些公共属性
     *
     * <p>当track的Properties，superProperties和SDK自动生成的automaticProperties有相同的key时，遵循如下的优先级：
     * track.properties 高于 superProperties 高于 automaticProperties
     *
     * <p>另外，当这个接口被多次调用时，是用新传入的数据去merge先前的数据
     *
     * <p>例如，在调用接口前，dict是 {"a":1, "b": "bbb"}，传入的dict是 {"b": 123, "c": "asd"}，则merge后 的结果是 {"a":1,
     * "b": 123, "c": "asd"}
     *
     ...
     * 记录用户注册事件
     *
     * <p>这个接口是一个较为复杂的功能，请在使用前先阅读相关说明: http://www.sensorsdata.cn/manual/track_signup.html
     * 并在必要时联系我们的技术支持人员。
     *
     * @param loginId 登录 ID
     ...
     * 并在必要时联系我们的技术支持人员。
     *
     * <p>属性取值可接受类型为{@link Number}, {@link String}, {@link java.util.Date}和{@link java.util.List}；
     ...
     */
    void trackSignUp(
            @NonNull String loginId, @NonNull String anonymousId, Map<String, Object> properties)
            throws InvalidArgumentException;
     ...
     * java.util.List}；
     *
     * <p>如果要设置的properties的key，之前在这个用户的profile中已经存在，则覆盖，否则，新创建
     *
     ...
     */
    void profileSet(
            @NonNull String distinctId, @NonNull boolean isLoginId, Map<String, Object> properties)
            throws InvalidArgumentException;
     ...
     * java.util.List}；
     *
     * <p>与profileSet接口不同的是： 如果要设置的properties的key，在这个用户的profile中已经存在，则不处理，否则，新创建
     *
     * @param userRecord 用户属性实体
     * @throws InvalidArgumentException 用户属性类型或者用户ID不合法则抛出该异常
     */
     ...
     */
    void itemDelete(
            @NonNull String itemType, @NonNull String itemId, Map<String, Object> properties)
            throws InvalidArgumentException;
     ...
     */
    void bind(@NonNull SensorsAnalyticsIdentity... analyticsIdentity)
            throws InvalidArgumentException;
     ...
     */
    @Deprecated
    void unbind(@NonNull SensorsAnalyticsIdentity analyticsIdentity)
            throws InvalidArgumentException;
     ...
     */
    void bind(
            Map<String, Object> properties, @NonNull SensorsAnalyticsIdentity... analyticsIdentity)
            throws InvalidArgumentException;
     ...
     * java.util.List}；
     *
     * <p>如果要设置的 properties 的 key，之前在这个用户的 profile 中已经存在，则覆盖，否则，新创建
     *
     * @param analyticsIdentity 用户标识 ID
     ...
     */
    void profileSetById(
            @NonNull SensorsAnalyticsIdentity analyticsIdentity, Map<String, Object> properties)
            throws InvalidArgumentException;
     ...
     * java.util.List}；
     *
     * <p>与 profileSetById 接口不同的是： 如果要设置的 properties 的 key，在这个用户的 profile 中已经存在，则不处理，否则，新创建
     *
     * @param analyticsIdentity 用户标识 ID
     * @param properties 用户属性
     * @throws InvalidArgumentException 不合法参数异常
