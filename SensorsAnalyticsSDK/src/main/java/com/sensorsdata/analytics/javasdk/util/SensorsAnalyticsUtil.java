...
public class SensorsAnalyticsUtil {
    public static final Pattern KEY_PATTERN =
            Pattern.compile(
                    "^((?!^distinct_id$|^original_id$|^event_id$|^time$|^properties$|^id$|^first_id$|^second_id$|^users$|^events$|^event$|^user_id$|^date$|^datetime$|^user_group|^user_tag)[a-zA-Z_$][a-zA-Z\\d_$]{0,99})$",
        ...
        for (Map.Entry<String, Object> property : properties.entrySet()) {
            if (property.getKey().equals("$is_login_id")) {
                if (!(property.getValue() instanceof Boolean)) {
                    throw new InvalidArgumentException(
                            "The property value of '$is_login_id' should be Boolean.");
                }
                continue;
            }
            ...
            if (!(property.getValue() instanceof Number)
                ...
                String type =
                        property.getValue() == null
                                ? "null"
                                : property.getValue().getClass().toString();
                throw new InvalidArgumentException(
                        String.format(
            ...
            if (property.getValue() instanceof List<?>
                ...
                for (final ListIterator<Object> it =
                                ((List<Object>) property.getValue()).listIterator();
                        it.hasNext(); ) {
                    Object element = it.next();
                    if (!(element instanceof String)) {
                        throw new InvalidArgumentException(
                                String.format(
                                        "The property '%s' should be a list of String.",
                                        property.getKey()));
                    }
            ...
            boolean isFilterKey =
                    property.getKey().equals(SensorsConst.PROJECT_SYSTEM_ATTR)
            ...
            if (eventType.equals(SensorsConst.PROFILE_INCREMENT_ACTION_TYPE)) {
                if (!isFilterKey) {
                    if (!(property.getValue() instanceof Number)) {
                        ...
                        throw new InvalidArgumentException(
                                String.format(
                                        "The property value of PROFILE_INCREMENT should be a Number.The current type is %s.",
                                        type));
                    }
                }
    ...
    public static void assertValue(String type, String value) throws InvalidArgumentException {
        if (value == null || value.length() < 1) {
            throw new InvalidArgumentException(
                    String.format("The %s value is empty or null.", type));
        }
        if (value.length() > 255) {
            throw new InvalidArgumentException(
                    String.format("The %s value %s is too long, max length is 255.", type, value));
        }
    }
    ...
    public static void assertKey(String type, String key) throws InvalidArgumentException {
        ...
        if (!(KEY_PATTERN.matcher(key).matches())) {
            throw new InvalidArgumentException(
                    String.format("The %s key '%s' is invalid.", type, key));
        }
    }
        ...
        try {
            if (propertyMap.containsKey(SensorsConst.TRACK_ID)) {
                return Integer.parseInt(
                        (String.valueOf(propertyMap.remove(SensorsConst.TRACK_ID))));
            }
    ...
    public static String checkUserInfo(
        ...
        if (userId != null) {
            if (distinctId != null) {
                ...
                return distinctId;
            } else return SensorsConst.USER_ID_PREFIX + userId;
        }
        Pair<String, Boolean> userPair =
                checkIdentitiesAndGenerateDistinctId(distinctId, identities);
        ...
        for (int i = 0; i < failedData.getFailedData().size(); i++) {
            ...
            switch (type) {
                ...
                case ITEM_DELETE_ACTION_TYPE:
                    if (verifyMapValueIsBlank(dataMap, "item_id")) {
                        throw new InvalidArgumentException(
                                String.format(
                                        "The index [%d] of failed data list have not [item_id].",
                                        i));
                    }
                    ...
                    if (verifyMapValueIsBlank(dataMap, "item_type")) {
                        throw new InvalidArgumentException(
                                String.format(
                                        "The index [%d] of failed data list have not [item_type].",
                                        i));
                    }
                ...
                case TRACK_SIGN_UP_ACTION_TYPE:
                    if (verifyMapValueIsBlank(dataMap, "original_id")) {
                        throw new InvalidArgumentException(
                                String.format(
                                        "The index [%d] of failed data list have not [original_id].",
                                        i));
                    }
                    assertValue("Original Distinct Id", dataMap.get("original_id").toString());
                case BIND_ID_ACTION_TYPE:
                case UNBIND_ID_ACTION_TYPE:
                    if (!TRACK_SIGN_UP_ACTION_TYPE.equals(type)
                            && verifyMapValueIsBlank(dataMap, "identities")) {
                        throw new InvalidArgumentException(
                                String.format(
                                        "The index [%d] of failed data list have not [identities].",
                                        i));
                    }
                ...
                case PROFILE_DELETE_ACTION_TYPE:
                    if (verifyMapValueIsBlank(dataMap, "distinct_id")) {
                        throw new InvalidArgumentException(
                                String.format(
                                        "The index [%d] of failed data list have not [distinct_id].",
                                        i));
                    }
                    if (verifyMapValueIsBlank(dataMap, "_track_id")
                            || !(dataMap.get("_track_id") instanceof Integer)) {
                        throw new InvalidArgumentException(
                                String.format(
                                        "The index [%d] of of failed data list is invalid.", i));
                    }
                ...
                default:
                    throw new InvalidArgumentException(
                            String.format(
                                    "The index [%d] of failed data list can not identify [action_type:%s].",
                                    i, type));
            }
            if (dataMap.containsKey("properties")) {
                if (!(dataMap.get("properties") instanceof Map)) {
                    throw new InvalidArgumentException(
                            String.format(
                                    "The index [%d] of failed data list can not identify [properties:%s].",
                                    i, type));
                }
    ...
    public static void assertSchema(String schema) throws InvalidArgumentException {
        ...
        if (!KEY_PATTERN.matcher(schema).matches()) {
            throw new InvalidArgumentException(
                    String.format("Thr schema '%s' is invalid.", schema));
        }
    }
        ...
        for (Map.Entry<String, Object> entry : properties.entrySet()) {
            ...
            if (PROFILE_INCREMENT_ACTION_TYPE.equals(actionType)) {
                if ((!SensorsConst.PROJECT_SYSTEM_ATTR.equals(entry.getKey())
                                && !SensorsConst.TOKEN_SYSTEM_ATTR.equals(entry.getKey()))
            ...
            if (PROFILE_APPEND_ACTION_TYPE.equals(actionType)) {
                if ((!SensorsConst.PROJECT_SYSTEM_ATTR.equals(entry.getKey())
                    ...
                    throw new InvalidArgumentException(
                            String.format(
                                    "The property value of PROFILE_APPEND should be a List<String>.The current type is %s.",
                                    value.getClass().getName()));
                }
            }
            // 处理 profile_unset
            if (PROFILE_UNSET_ACTION_TYPE.equals(actionType)) {
                if (!(value instanceof Boolean) || !Boolean.parseBoolean(value.toString())) {
                    throw new InvalidArgumentException(
                            "The property value of " + entry.getKey() + " should be true.");
                }
            }
        }
    }
    ...
    public static void assertEventItemPair(Pair<String, String> itemPair)
            throws InvalidArgumentException {
        if (itemPair == null) {
