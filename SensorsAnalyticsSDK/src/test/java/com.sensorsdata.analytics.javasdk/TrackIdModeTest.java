...
public class TrackIdModeTest extends  SensorsBaseTest{
  private final static Map<String, Object> mp = new HashMap<>();
  ...
  /**
   * 测试功能：上报事件到 event 表
   * 测试预期：生成的数据符合神策格式,且 _track_id 存在，properties 中不包含 $time 和 $track_id
   */
  @Test
  ...
  /**
   * 功能：检验 IDM 下上报事件属性
   * 预期：生成的数据符合神策格式,且 _track_id 存在，properties 中不包含 $time 和 $track_id
   */
  @Test
  ...
  public void profileSet() throws InvalidArgumentException {
    UserRecord userRecord = UserRecord.builder().setDistinctId(cookieId)
        .addProperties(mp)
  ...
  /**
   * 功能：IDM 下校验记录单个用户的属性信息
   * 预期：生成的数据符合神策格式,且 _track_id 存在，properties 中不包含 $time 和 $track_id
   */
  @Test
  public void profileSetById() throws InvalidArgumentException {
    IDMUserRecord userRecord = IDMUserRecord.starter().setDistinctId(cookieId)
        .addProperties(mp)
        .build();
  ...
  public void profileUnsetById() throws InvalidArgumentException {
    ...
    ma.put("Channel", true);
    IDMUserRecord idmUserRecord = IDMUserRecord.starter().setDistinctId(cookieId)
        .addProperties(ma)
        .build();
  ...
  public void profileAppend() throws  InvalidArgumentException {
    ...
    UserRecord appendRecord = UserRecord.builder().setDistinctId(cookieId).isLoginId(Boolean.TRUE)
  ...
  /**
   * 功能：IDM 下校验为指定用户的一个或多个数组类型的属性追加字符串
   * 预期：生成的数据符合神策格式,且 _track_id 存在，properties 中不包含 $time 和 $track_id
   */
  @Test
  public void profileAppendById() throws InvalidArgumentException {
    ...
    Map<String, Object> mp = new HashMap<>();
    mp.put("interest", newInterest);
    IDMUserRecord idmUserRecord = IDMUserRecord.starter().setDistinctId(cookieId).addProperty("interest", newInterest).build();
  ...
  public void profileIncrement() throws InvalidArgumentException {
    Map<String, Object> mp = new HashMap<>();
    mp.put("age",2);
    UserRecord incrementRecord = UserRecord.builder().setDistinctId(cookieId).isLoginId(Boolean.TRUE)
        .addProperty("age", 2)
        .build();
  ...
  /**
   * 功能：IDM 下校验为指定用户的一个或多个数值类型的属性累加一个数值
   * 预期：生成的数据符合神策格式,且 _track_id 存在，properties 中不包含 $time 和 $track_id
   */
  @Test
  public void profileIncrementById() throws InvalidArgumentException {
    IDMUserRecord idmUserRecord = IDMUserRecord.starter().setDistinctId(cookieId).addProperty("age",2).build();
    Map<String, Object> mp = new HashMap<>();
    mp.put("age",2);
  ...
  /**
   * 功能：校验增加 item 记录
   * 预期：生成的数据符合神策格式,且 _track_id 不存在，properties 中不包含 $time 和 $track_id
   */
  @Test
  public void itemSet() throws InvalidArgumentException {
    ...
    ItemRecord itemRecord = ItemRecord.builder().setItemType("test").setItemId(cookieId).addProperties(mp).build();
