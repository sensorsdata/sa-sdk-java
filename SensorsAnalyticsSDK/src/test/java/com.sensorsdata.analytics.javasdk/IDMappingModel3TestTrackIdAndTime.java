...
/**
 * 适用于 v3.4.4+ 版本 测试点：验证事件的 _track_id 正常由 $track_id 生成，time 由 $time 生成。 无特殊情况，不在下面的 testcase 上一一说明
 */
public class IDMappingModel3TestTrackIdAndTime extends SensorsBaseTest {
    ...
    private void assertNotNullProp() {
        Assertions.assertNotNull(messageList.get(0).get("identities"));
    ...
    public void testSingleIdentity() throws InvalidArgumentException {
        ...
        Assertions.assertEquals(date.getTime(), messageList.get(0).get("time"));
    ...
    /**
     * 【已知问题】https://jira.sensorsdata.cn/browse/SDK-4863 【Java SDK】【_track_id】公共属性设置 $time, 属性中也带
     * $time，同时这条事件的 time 也会由 $time 生成
     */
    @Test
    @Disabled
    public void checkTrackByIdSuperProperties() throws InvalidArgumentException {
        ...
        Assertions.assertNotEquals(
                111, messageList.get(0).get("_track_id")); // 公共属性设置 $track_id 不生效的
        Assertions.assertNotEquals(
                date.getTime(), messageList.get(0).get("time")); // 公共属性设置 $time 不生效的

        Map<String, Object> props = (Map<String, Object>) messageList.get(0).get("properties");
        Assertions.assertFalse(props.containsKey("$track_id")); // properties 不包含 $track_id
        Assertions.assertTrue(props.containsKey("abc")); // properties 包含其他自定义属性
    }
    ...
    public void checkTrackByIdSuperProperties01() throws InvalidArgumentException {
        ...
        Assertions.assertNotEquals(
                "aaa", messageList.get(0).get("_track_id")); // 公共属性设置 $track_id 不生效的
        Assertions.assertNotEquals(
                date.getTime(), messageList.get(0).get("time")); // 公共属性设置 $time 不生效的
    ...
    /**
     * v3.4.2 后新增接口 trackById(@NonNull IDMEventRecord idmEventRecord) 测试 测试点：IDMEventRecord 传入正常
     * distinctId，identityMap 传入 $identity_login_id
     */
    @Test
    public void testTrackByIdIDMEventRecord() throws InvalidArgumentException {
        Date date = new Date();
        IDMEventRecord record =
        ...
        Assertions.assertEquals(
                date.getTime(), messageList.get(0).get("time")); // 公共属性设置 $track_id 不生效的
    ...
    public void testProfileSetById() throws InvalidArgumentException {
        ...
        Map<String, Object> props = (Map<String, Object>) messageList.get(0).get("properties");
        Assertions.assertFalse(props.containsKey("$track_id")); // properties 不包含 $track_id
        Assertions.assertTrue(props.containsKey("age")); // properties 包含其他自定义属性
    }
    ...
    public void testProfileIncrementById00() {
        try {
            ...
            Assertions.assertEquals(
                    "The property value of PROFILE_INCREMENT should be a Number.The current type is class java.util.Date.",
                    e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void testProfileAppendById() {
        try {
            ...
            Assertions.fail("profileAppendById should throw InvalidArgumentException.");
        } catch (InvalidArgumentException e) {
    ...
    public void testProfileUnsetById() {
        try {
            ...
            Assertions.fail("profileUnsetById should throw InvalidArgumentException.");
        } catch (InvalidArgumentException e) {
        ...
        try {
            userRecord =
                    IDMUserRecord.starter()
                            .setDistinctId("xc001") // 手动指定外层 distinct_id
                            .addIdentityProperty(
                                    SensorsAnalyticsIdentity.LOGIN_ID, "dis123") // 用户维度标识
                            .addIdentityProperty(
                                    SensorsAnalyticsIdentity.EMAIL, "123@qq.com") // 用户维度标识
                            .addProperty("age", 1) // 设置埋点事件属性
                            .addProperty("$track_id", 111)
                            .addProperty("$time", date) // 设置 $track_id
                            .build();
